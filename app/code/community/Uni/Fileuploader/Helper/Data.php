...
class Uni_Fileuploader_Helper_Data extends Mage_Core_Helper_Abstract {
	...
	public function getClassName($item){
        ...
        $videoContent = $item['file_content'];
        //attached file
        if($uploadedFile){
            //get extension
            preg_match("/[^\.]*$/", $item['uploaded_file'], $output_array);
            if($output_array[0]){
                $ext = $output_array[0];
                //ppt
                if($ext == "ppt" || $ext == "pptx"){
                    return "resourcePPTImageHolder";
                }
                //pdf
                else if($ext == "pdf"){
                    return "resourcePDFImageHolder";
                }
                //Images
                else if ($ext == 'jpg' || $ext == 'jpeg' || $ext == 'gif' || $ext == 'png' || $ext == 'bmp') {
                    return "resourceImageTypeHolder";
                }
                //Doc and docx
                else if($ext == 'doc' || $ext == 'docx'){
                    return "resourceDocImageHolder";
                }         
                //audio files
                else if($ext == 'wmv' || $ext == 'wav' || $ext == 'mp3') {
                    return "resourceAudioImageHolder";
                }
            }
        }
        //Video
        else if($videoContent){
            return "resourceVideoTypeHolder";
        }
        //if no extension/no registered extension then considering as other files
        return "resourceOtherImageHolder";
    }

    public function attachLink($item){
        if($item['uploaded_file']){
            return $this->getDownloadFileUrl($item['uploaded_file'], $item['content_disp']); 
        }
        else if($item['file_content']){
            return strip_tags ($item['file_content']);
        }
        return false;
    }

    public function getFileAttachment($_productId){
        $attachments = Mage::getModel('fileuploader/fileuploader')->getFilesByProductId($_productId);
        $attachItems = $attachments['items'];
        $resourceAttachments =  array();
        $_fileAttachments = array();
        for($i = 0; $i < count($attachItems); $i++){
            $item = $attachItems[$i];
            if($item['content_disp'] == '0'){
                $_fileAttachments[] = $item;
            }
        }
        if (count($_fileAttachments) > 0){
            $attachment = $_fileAttachments[0];
            return $this->getDownloadFileUrl($attachment['uploaded_file'], $attachment['content_disp']);
        }
        return false;
    }
}
